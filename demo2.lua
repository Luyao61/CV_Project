...
local count = 0
local accuracy = 0
--parse labeled txt file
local file_io = io.open("data/" .. file .. ".txt")
local labelset = {}
if file_io then
  for line in file_io:lines() do
    table.insert(labelset, tonumber(line))
  end
end
...
model = torch.load("models/AlexNet_W_epoch20_new.t7")
...
if paths.dir("Alexnet_Output_new/" .. file .. "/") == nil then
  paths.mkdir("Alexnet_Output_new/" .. file.. "/")
end
for i = 1, frames:size(),batchSize do
  ...
  for j = 1, outputs:size(1) do
      ...
      --print(classIds[1], labelset[math.floor(i/10)+1], count)
      if (classIds[1] == labelset[math.floor(i/10)+1]) then
        count = count + 1
      end

      image.save("Alexnet_Output_new/" .. file .. "/" .. (i+j-1) .. ".jpg", frames.data[i+j-1])
  end
end
